...
clr.AddReference('KirbyLib')
clr.AddReference('ArchipelagoStepPatcher')
...
#APSPBridge = ctypes.CDLL("\ArchipelagoStepPatcher")
...
class KRtDLCommandProcessor(ClientCommandProcessor):
    ...
    def _cmd_setdolphintoolpath(self):
        """Set the path to DolphinTool.exe."""
        KRtDLWorld.settings.dolphin_tool_path = Utils.open_filename("DolphinTool", ((".exe", (".exe",)),))

    def _cmd_setgamepath(self):
        """Set the path to your US copy of Kirby's Return to Dream Land.
        Must be in the formats .iso, .rvz or .wbfs."""
        KRtDLWorld.settings.game_path = Utils.open_filename("Kirby's Return to Dream Land (USA) (En,Fr,Es)", (("Wii Disc", (".iso", ".rvz", ".wbfs")),))
    
    def _cmd_patch(self):
        """Patches a copy of the game using a .apkrtdl file."""
        APKRTDLInput = Utils.open_filename("KRtDL AP Patch File", (("KRtDL AP Patch File", (".apkrtdl",)),))
        OutputDirectory = Utils.open_directory("Folder")

        #with zipfile.ZipFile(APKRTDLInput) as ZipArchive:
            #with ZipArchive.open('config.json', 'r') as ConfigFile:
                #FileContents = json.load(ConfigFile)
                #print(FileContents["items"])
        
        PatchingInstance = StepPatcher(KRtDLWorld.settings.dolphin_tool_path, KRtDLWorld.settings.game_path)
        PatchingInstance.Extract()

        with zipfile.ZipFile(APKRTDLInput) as ZipArchive:
            with ZipArchive.open('config.json', 'r') as ConfigFile:
                FileContents = json.load(ConfigFile)
                FileContents = str(FileContents["items"]).replace('\'', '"')
                print(FileContents)
                PatchingInstance.PatchItems(FileContents)
        
        PatchingInstance.PatchItems(TheJSONVoorhees)
        PatchingInstance.Rebuild()
...
class KRtDLContext(CommonContext):
    ...
    def __init__(self, server_address: str, password: str, krtdl_file: Optional[str] = None):
        ...
        if not KRtDLWorld.settings.dolphin_tool_path:
            KRtDLWorld.settings.dolphin_tool_path = Utils.open_filename("DolphinTool", ((".exe", (".exe",)),))
...
async def handle_check_goal_complete(ctx: KRtDLContext):
    hasmagolorgoal: bool = false
    haslandiagoal: bool = false
    hasdoomergoal: bool = false
